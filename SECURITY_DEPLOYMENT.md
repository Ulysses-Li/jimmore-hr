...
本功能採 fail-closed：Functions 尚未部署、App Check 尚未設定或沒有公司據點時，安全打卡會被拒絕，不會退回前端直接寫入模式。系統不再要求 Passkey 或生物辨識，因此無法以裝置本人驗證降低代打卡風險。
...
- 確認瀏覽器無法直接寫入 `attendance`、`attendanceDaily`、`attendanceExceptions`、`auditEvents`、審核結果、員工假別餘額與 `workSettings`。
- 主管只能讀取與審核同部門案件，管理員才能補登。
- 每週查看 Functions 錯誤率、App Check 無效請求、Firestore 用量與未處理／逾期案件。
- Web GPS 仍可能被具備特殊工具的人偽造；移除生物辨識後，帳號密碼也可能被交給他人使用。據點限制、外勤配置及不可變稽核只能降低風險，不能防止代打卡。
